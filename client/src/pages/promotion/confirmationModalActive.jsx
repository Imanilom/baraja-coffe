...
const ConfirmationModalActive = ({ isOpen, onClose, onConfirm, voucher, newStatus }) => {
  const dateNow = new Date();
  const validToDate = voucher?.validTo ? new Date(voucher.validTo) : null;
        ...
        {validToDate < dateNow ? (
          <>
            ...
            <p className="text-sm text-gray-600 mb-6 leading-relaxed">
              Maaf {" "}
              <span className="font-medium text-gray-800">{voucher?.name}</span>{" "}
              tidak dapat diaktifkan karena sudah melawati batas waktu yang di tentukan
            </p>
          ...
          <>
            {/* Body */}
            <p className="text-sm text-gray-600 mb-6 leading-relaxed">
              Apakah Anda yakin ingin mengubah status voucher{" "}
              <span className="font-medium text-gray-800">{voucher?.name}</span>{" "}
              menjadi{" "}
              <span
