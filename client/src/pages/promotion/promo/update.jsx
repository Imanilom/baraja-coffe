...
const UpdatePromoPage = () => {
    ...
    const fetchPromo = async () => {
        try {
            ...
            setPromo({
                ...
                outlet: (data.outlet || []).map(o => typeof o === "object" ? o._id : o),
                validFrom: data.validFrom || new Date().toISOString().split("T")[0],
                validTo: data.validTo || new Date().toISOString().split("T")[0]
            })
        } catch (error) {
    ...
    const handleOutletChange = (e) => {
        ...
        setPromo((prevPromo) => {
            let updatedOutlets = checked
                ? [...prevPromo.outlet, value]
                : prevPromo.outlet.filter((id) => id !== value);

            return {
                ...prevPromo,
                outlet: updatedOutlets
            };
        });
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            setError(null);
            const updatedData = { ...promo };
            await axios.put(`/api/promotion/promos/${id}`, updatedData);
            ...
            <form onSubmit={handleSubmit}>
                <div className="px-3 py-2 flex justify-between items-center border-b">
                    <div className="flex items-center space-x-2">
                        ...
                        <FaChevronRight className="text-[15px] text-gray-500" />
                        <Link to="/admin/promo-khusus-create" className="text-[15px] text-gray-500">{promo.name}</Link>
                    </div>
