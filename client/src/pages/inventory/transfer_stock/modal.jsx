        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 z-50 flex items-center justify-center">
            <div className="relative bg-white rounded shadow-lg w-full max-w-lg max-h-[400px] flex flex-col overflow-hidden">
                ...
                <div className="flex justify-between items-center px-6 py-4 border-b">
                    <h2 className="text-lg font-semibold">Impor Transfer Stok</h2>
                ...
                <div className="px-6 py-4 overflow-y-auto flex-1">
                    <form onSubmit={onSubmit} className="space-y-4"> {/* Padding bawah supaya isi tak tertutup footer */}
                        ...
                        <div>
                            <label className="text-sm text-gray-600 mb-1 block">2. Pilih Outlet Tujuan</label>
                            <select className="w-full border px-3 py-2 rounded text-sm">
                                <option value="">- Pilih Outlet -</option>
                                {outlets.map((outlet, idx) => (
                                    <option key={idx} value={outlet.id}>{outlet.name}</option>
                        ...
                        {/* 2. Upload File */}
                        <div>
                            <label className="text-sm text-gray-600 block mb-1">3. Pilih file XLS yang sudah Anda input data</label>
                            <input type="file" className="w-full border px-3 py-2 rounded text-sm" />
                        </div>

                        {/* 3. Tanggal */}
                        <div>
                            <label className="text-sm text-gray-600 block mb-1">4. Tanggal</label>
                        ...
                        <div>
                            ...
                            <input type="email" className="w-full border px-3 py-2 rounded text-sm" />
                        </div>

                        {/* 5. Catatan */}
                        <div>
                            <label className="text-sm text-gray-600 block mb-1">6. Catatan</label>
                            <textarea className="w-full border px-3 py-2 rounded text-sm" placeholder="Ketik Catatan..."></textarea>
                        </div>
                    </form>
                </div>
