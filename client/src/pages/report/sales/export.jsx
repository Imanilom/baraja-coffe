...
//             <div className="bg-white rounded-md shadow-2xl p-8 max-w-7xl w-full relative h-[60vh] overflow-y-auto">
//                 {/* Tombol Tutup */}
...
const ExportFilter = ({ isOpen, onClose }) => {
    ...
    // Fungsi untuk fetch payment details
        ...
        try {
            ...
            const ordersWithPayment = await Promise.all(
                orderData.map(async (order) => {
                    try {
                        const paymentDetails = await fetchPaymentDetails(order.order_id);
                        return {
                            ...order,
                            paymentDetails: paymentDetails || null,
                            actualPaymentMethod: paymentDetails?.method || order.paymentMethod || 'N/A'
                        };
                    } catch (err) {
                        ...
                        return {
                            ...order,
                            paymentDetails: null,
                            actualPaymentMethod: order.paymentMethod || 'N/A'
                        };
                    }
                })
            );
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            const exportData = filtered.flatMap((order) => {
                ...
                // Ambil payment method dari payment details
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-md shadow-2xl p-8 max-w-7xl w-full relative h-[60vh] overflow-y-auto">
                ...
                {/* Loading indicator saat fetch orders */}
                {orders.length === 0 && isOpen ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                            <p className="text-xs text-blue-800">
                                💡 <strong>Info:</strong> Jika tidak ada filter yang dipilih, semua data dalam rentang tanggal akan diekspor.
                                Data payment method akan diambil dari sistem pembayaran yang sebenarnya.
                            </p>
                        </div>
                        ...
                        <div className="pt-4">
                            <button
                                ...
                                className={`w-full py-2 px-4 rounded-lg text-sm font-medium transition flex items-center justify-center gap-2 ${isExporting || orders.length === 0
                                    ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                                    : 'bg-white hover:bg-[#005429] hover:text-white border-[#005429] border text-[#005429]'
                                    }`}
                            >
