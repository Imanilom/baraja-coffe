...
export default function socketHandler(io) {
    ...
    io.on('connection', (socket) => {
        ...
        // ✅ OPTIMASI 3: Lightweight authentication (minimal DB queries)
        socket.on('device_authenticate_session', async (data, callback) => {
            try {
                const { sessionId, deviceId } = data;

                // ✅ Single query dengan select minimal
                const session = await DeviceSession.findOne({
                    _id: sessionId,
                    device: deviceId,
                    isActive: true
                })
                    .select('device user outlet role socketId')
                    .populate('device', 'deviceId deviceName assignedAreas assignedTables orderTypes location')
                    .populate('user', 'name')
                ...
                await socketManagement.registerDevice(socket, {
                ...
                if (session.role.includes('bar')) {
                    ...
                    rooms.push(`bar_${barType}`);
                }

                if (session.role.includes('kitchen')) {
                    rooms.push('kitchen_room', `kitchen_${session.outlet._id}`);
                }

                // ✅ Join semua room sekaligus
                rooms.forEach(room => socket.join(room));

                console.log(`✅ Device ${session.device.deviceName} joined ${rooms.length} rooms`);
        ...
        // ✅ OPTIMASI 5: Direct emit untuk new order (no DB query)
        socket.on('new_order_created', async (orderData) => {
            try {
                console.log('🆕 New order:', orderData.order_id);

                // ✅ Emit LANGSUNG ke semua room yang relevan
                const rooms = ['cashier_room', 'kitchen_room'];

                // Add bar rooms jika ada beverage items
                if (orderData.items?.some(item => item.menuItem?.workstation === 'bar')) {
                    rooms.push('bar_depan', 'bar_belakang');
                }

                // Add area rooms
                if (orderData.tableNumber) {
                    const areaCode = orderData.tableNumber.charAt(0).toUpperCase();
                    rooms.push(`area_${areaCode}`);
                }

                // ✅ Broadcast ke semua room sekaligus
                rooms.forEach(room => {
                ...
                console.log(`✅ New order ${orderData.order_id} broadcasted to ${rooms.length} rooms`);
        ...
        // ✅ OPTIMASI 6: Lightweight disconnection
            ...
            setImmediate(async () => {
                try {
                    const session = await DeviceSession.findOne({
                        socketId: socket.id,
                        isActive: true
                    }).select('device user outlet').lean();
