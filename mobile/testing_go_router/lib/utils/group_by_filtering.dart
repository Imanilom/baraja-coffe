...
List<Map<String, dynamic>> searchAndFilterProducts(
  ...
  var filteredProducts =
      allProducts
          .where((product) => product.mainCategory!.contains(category))
          .toList();
  ...
  var groupedBySubCategory = groupBy(
    filteredProducts,
    (MenuItemModel product) => product.name,
  );

  // 4️⃣ Konversi ke format yang bisa digunakan di UI
  return groupedBySubCategory.entries.map((entry) {
    return {"subCategory": entry.key, "products": entry.value};
  }).toList();
}
