...
class _DateSelectorState extends State<DateSelector> {
  ...
  List<Widget> _generateDaysInMonth() {
    ...
    for (int i = 0; i < firstWeekdayOfMonth; i++) {
      dayWidgets.add(
        Container(width: 36, height: 36, margin: const EdgeInsets.all(2)),
      );
    }
            ...
            decoration: BoxDecoration(
              ...
              border:
                  isToday && !isSelected && !isPast
                      ? Border.all(color: Color(0xFF007339))
                      : null,
            ),
            child: Center(
              child: Text(
  ...
  Widget _dayLabel(String day) {
    return Container(
      ...
      child: Text(day, style: TextStyle(fontSize: 14, color: Colors.grey[600])),
    );
  }
